...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
	...
	// printf("%d\n", val);	// activate RTOS, dispatch print to Task1
    if (hadc->Instance == ADC1)
    {
        uint16_t val = HAL_ADC_GetValue(&hadc1);
        osMessageQueuePut(ADCQueueHandle, &val, 0U, 0U);
    }
}
/* USER CODE END 0 */
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
	  if (osMessageQueueGet(ADCQueueHandle, &val, NULL, osWaitForever) == osOK)
	  {
		  Vsense = ((float)val) * 3.3f / 4095.0f;
