...
/* USER CODE BEGIN PTD */
typedef enum {
	BUFFER_TOP_HALF_FULL = 0,
	BUFFER_BOTTOM_HALF_FULL = 1
}TransEvent_t;
...
uint16_t sample_buffer[SAMPLE_BUFFER_SIZE];	// CPU read this
/* USER CODE END PV */
  ...
  /* Infinite loop */
    ...
    for(;;)
    {
        ...
        if (osMessageQueueGet(myQueue01Handle, &flag, NULL, osWaitForever) == osOK)
        {
            ...
            else if (flag == BUFFER_BOTTOM_HALF_FULL)
            {
