...
uint8_t RemoteIP[] = {192,168,229,61};
...
int main(void)
{
  ...
  int16_t pDataXYZ[3];   // accelero X, Y, Z
  int i;
  ...
  BSP_ACCELERO_Init();
  //***added
  uint8_t streaming = 1;  // 1->send data to sensor, 0->stop
  TERMOUT("> Start main loop.\n");

  while(1)
  {
    if(Socket != -1)
    {
        // 1. check if received message from Python
        ret = WIFI_ReceiveData(Socket, RxData, sizeof(RxData)-1, &Datalen, 200);
        if (ret == WIFI_STATUS_OK && Datalen > 0)
        {
            ...
            {
                streaming = 0;
                TERMOUT("Sensor streaming stopped.\n");
            }
            else if (strncmp((char*)RxData, "start", 5) == 0)
            {
                streaming = 1;
                TERMOUT("Sensor streaming resumed.\n");
            }
            else
            {
                TERMOUT("Command ignored.\n");
            }
        }

        // 2. if streaming == 1, send data of accelero
        if (streaming)
        {
            int16_t pDataXYZ[3];
            BSP_ACCELERO_AccGetXYZ(pDataXYZ);
            sprintf((char*)TxData, "X:%d Y:%d Z:%d\r\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);

            ret = WIFI_SendData(Socket, TxData, strlen((char*)TxData), &Datalen, WIFI_WRITE_TIMEOUT);
            if (ret != WIFI_STATUS_OK)
            {
                TERMOUT("> ERROR: Failed to Send Data (%d)\r\n", ret);
                break;
            }
  ...
  {
      BSP_ACCELERO_AccGetXYZ(pDataXYZ);
      sprintf(TxData, "X:%d Y:%d Z:%d\r\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);

      // send TCP Server to Python
      ret = WIFI_SendData(Socket, TxData, strlen((char*)TxData), &Datalen, WIFI_WRITE_TIMEOUT);
      if (ret != WIFI_STATUS_OK)
      {
