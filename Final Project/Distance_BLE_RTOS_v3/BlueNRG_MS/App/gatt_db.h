...
void BLE_Send_Alert_Status(uint8_t status); // 0:Safe, 1:Alert
void Read_Request_CB(uint16_t handle);
void Write_Request_CB(uint16_t handle, uint8_t data_length, uint8_t* data);
extern uint16_t SentryServiceHandle;
extern uint16_t SentryControlCharHandle; // 手機寫入 (開/關)
extern uint16_t SentryStatusCharHandle;  // MCU 通知 (警報)
