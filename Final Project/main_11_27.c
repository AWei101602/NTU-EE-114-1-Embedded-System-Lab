    ...
    if (htim->Instance == TIM2 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_3)
    {
        ...
        else
        {
            ...
            if (echo_end >= echo_start) {
                ...
                // 如果跨越了週期 (雖然 100ms 週期很長，不太可能發生，但寫著保險)
                echo_duration = (__HAL_TIM_GET_AUTORELOAD(htim) - echo_start) + echo_end;
            }
...
//  HAL_TIM_PWM_Start(&htim2,TIM_CHANNEL_1);
  // 1. 啟動 TIM2 的 PWM (負責自動發送 Trig)
  // 只要這一行，PA0 就會開始無止盡地每 100ms 發送一次訊號，完全不用管它
  // 1. 啟動 TIM2 CH1 的 PWM (自動發送 Trig)
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1);

  // 2. 啟動 TIM2 CH3 的 輸入捕獲 (自動監聽 Echo)
  // 記得要用 _IT 來開啟中斷
  HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_3);

  printf("TIM2 Dual Mode (PWM+IC) Started!\r\n");

  printf("Auto PWM HC-SR04 System Ready!\r\n");
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	    /* USER CODE END WHILE */
		  if (is_data_ready)
		      {
		          is_data_ready = 0; // 清除旗標

		          // 計算距離
		          distance = (float)echo_duration * 0.0343f / 2.0f;

		          // 簡單過濾
		          if (distance > 2.0f && distance < 400.0f) {
		              printf("Dist: %.2f cm (Time: %lu us) (Tick Time: %lu us) \r\n", distance, echo_duration,tick_duration);
		          } else {
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : SPSGRF_915_SDN_Pin */
...
//		else {
//			// 按鈕放開
//			tick_duration = osKernelGetTickCount() - tick_start;
//
//		}
//	}
//}
